...
# Two sudden tree mutations create three clear regimes in the trace variant
# distribution. Stacking more starts thrashing the tree-search step on small
# activity pools, so we keep it at two.
DRIFTS = [
    {"perspective": "control_flow", "subtype": "tree_mutation", "drift_type": "sudden", "change_proportion": 0.2},
    {"perspective": "control_flow", "subtype": "tree_mutation", "drift_type": "sudden", "change_proportion": 0.2},
]
...
if __name__ == "__main__":
    for index in range(NUM_LOGS):
        stem = f"control_flow_{index + 1:03d}"
