...
def test_intra_only_requested_features(log):
    fs = kairo.build_features(log, "intra_case", features=("progress", "current"))
...
def test_intra_prefix_semantics():
    ...
    fs = kairo.build_features(log, "intra_case", features=("activity_freq", "bigram", "vocab", "progress"))
    m = fs.frame
...
def test_resource_share_denominators_ignore_filter(log):
    full = kairo.build_features(log, "resource", features=("ho",), window_minutes=1440)
    part = kairo.build_features(log, "resource", features=("ho",), window_minutes=1440,
                                resources=("r0", "r1"))
...
def test_inter_attribute_features(log):
    fs = kairo.build_features(log, "inter_case", window_minutes=1440,
